...
public class FuZaLianBiaoDeFuZhiLcof {
    ...
    class Solution {
        public Node copyRandomList(Node head) {
            if (head==null)
                return null;
            Node coped=new Node(head.val);
            Node copedCur=coped;
            Node headCur=head;
            HashMap<Node,Node> map=new HashMap<>();
            map.put(headCur,copedCur);
            while (headCur.next!=null){
                copedCur.next = new Node(headCur.next.val);
                headCur = headCur.next;
                copedCur = copedCur.next;
                map.put(headCur,copedCur);
            }
            headCur=head;
            copedCur=coped;
            while (headCur!=null){
                if (headCur.random!=null)
                    copedCur.random=map.get(headCur.random);
                copedCur=copedCur.next;
                headCur=headCur.next;
            }
            return coped;
