...
public class BaZiFuChuanZhuanHuanChengZhengShuLcof {
    ...
    class Solution {
        public int strToInt(String str) {
            if (str==null)
                return 0;
//                throw new NullPointerException("input str is null");
            str=str.trim();
            if (str.length()==0)
                return 0;
//                throw new IllegalArgumentException("input str is empty");
            int signal=1;
            if (str.charAt(0)=='-'){
                signal=-1;
                str=str.substring(1);
            ...
            for (int i = 0; i < str.length(); i++) {
                if (Character.isDigit(str.charAt(i))){
                    ...
                    if (ans*signal>Integer.MAX_VALUE)
                        return Integer.MAX_VALUE;
                    else if (ans*signal<Integer.MIN_VALUE)
